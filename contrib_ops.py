...
def _to_3d(tensor):
  if tensor.get_shape().ndims != 2:
    raise ValueError("`tensor` must be a 2D Tensor, got %s" % tensor.get_shape())
  m, n = tensor.get_shape()
  print "m,n", m,n
  reshaped = tf.expand_dims(tensor, 2)
...
def highway_maxout(hidden_size, pool_size):
  ...
  def compute(u_t, h, u_s, u_e):
    ...
    state_s = tf.concat([h, u_s, u_e], 1)
    r = tf.tanh(batch_linear(state_s, hidden_size, False, name='r'))
    print "u_t", u_t
    print "r", r
    print "state_s", state_s
    print "hidden_size", hidden_size
    print "h, u_s, u_e", h, u_s, u_e
    u_r = tf.concat([u_t, r], 1)
    # first maxout
